...
import { Check, Clock, Sparkles, Brain, Timer, TrendingUp, ArrowRight } from "lucide-react";
            ...
            if (res.ok) {
              ...
              setTopics((prev) => [
                ...prev,
                {
                  id: saved.id,
                  title: saved.title,
                  nextRevisionDate: saved.nextRevisionDate,
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8 md:py-16 flex flex-col gap-8 sm:gap-16">
      <header className="flex flex-col items-center text-center">
        ...
        <div className="min-h-[140px] sm:min-h-[120px] flex items-center justify-center relative w-full max-w-3xl mt-2 px-2 sm:px-4">
      ...
      {!isAuthenticated && (
        <section className="flex flex-col gap-6">
          <h2 className="text-2xl font-bold tracking-tight text-center">Why ReviseFlow?</h2>
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              {
                icon: <Brain className="w-7 h-7 text-purple-500" />,
                title: "Spaced Repetition",
                desc: "Science-backed scheduling that cements knowledge into permanent memory.",
                bg: "bg-purple-50 dark:bg-purple-900/20",
              },
              {
                icon: <Timer className="w-7 h-7 text-red-500" />,
                title: "Pomodoro Timer",
                desc: "Focused work intervals with smart breaks to prevent burnout.",
                bg: "bg-red-50 dark:bg-red-900/20",
              },
              {
                icon: <TrendingUp className="w-7 h-7 text-emerald-500" />,
                title: "Track Progress",
                desc: "Visual timeline of your learning journey and revision history.",
                bg: "bg-emerald-50 dark:bg-emerald-900/20",
              },
            ].map((feature) => (
              <div
                key={feature.title}
                className="flex flex-col items-center text-center gap-3 p-6 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl border border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-sm hover:shadow-md transition-all"
              >
                <div className={`p-3 rounded-xl ${feature.bg}`}>{feature.icon}</div>
          ...
          <div className="flex justify-center">
            ...
            >
              Learn the science behind it <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        </section>
      )}
        ...
        <section className="flex flex-col gap-6">
          ...
          {topics.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">
              {topics.map((topic) => {
                const overdue = isOverdue(topic.nextRevisionDate);
                return (
                  <div
                    key={topic.id}
                    className={`flex items-center justify-between p-5 bg-white dark:bg-neutral-900 border rounded-2xl shadow-sm hover:shadow-md transition-all group ${overdue ? "border-red-200 dark:border-red-900/50 bg-red-50/30 dark:bg-red-950/20" : "border-neutral-200 dark:border-neutral-800"}`}
                  >
                    <div className="flex flex-col gap-1 min-w-0 flex-1 pr-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-lg font-semibold break-words">{topic.title}</h3>
                        {overdue && (
                          <span className="text-[10px] font-bold uppercase tracking-wider bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-md shrink-0">
                            Overdue
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-1.5 text-xs font-medium text-neutral-500 dark:text-neutral-400">
                        ...
                        Revision #{topic.revisionCount + 1}
                      </div>
                    </div>
                    <button
                      onClick={() => handleMarkDone(topic.id, topic.title)}
                      disabled={loadingId === topic.id}
                      className="w-12 h-12 flex items-center justify-center rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-400 hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 dark:hover:text-green-400 transition-all disabled:opacity-50 group-hover:scale-105 active:scale-95 shrink-0"
