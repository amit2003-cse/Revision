...
import { ChevronDown, Calendar, Brain, Clock, Trash2, CheckCircle2, Circle, Power, Edit2, Search } from "lucide-react";
...
export function HistoryClient({ initialTopics }: { initialTopics: TopicHistory[] }) {
  ...
  const getTimeline = (topic: TopicHistory) => {
    ...
    return intervals.map((gap, index) => {
      ...
      if (isNext) {
        displayDate = new Date(topic.nextRevisionDate);
      } else {
        displayDate = new Date(baseDate);
        displayDate.setDate(displayDate.getDate() + cumulativeDays);
      }
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8 md:py-16 flex flex-col gap-8 md:gap-12 relative min-h-[85vh]">
      ...
      <div className="flex items-center justify-between text-sm text-neutral-500 dark:text-neutral-400">
        <span className="font-medium">{filteredTopics.length} topic{filteredTopics.length !== 1 ? "s" : ""}</span>
        {(searchTerm || filterStatus !== "all") && (
          <button onClick={() => { setSearchTerm(""); setFilterStatus("all"); }} className="text-blue-500 hover:underline font-bold text-xs">
            Clear filters
          </button>
        )}
      </div>
        ...
        <div className="flex flex-col gap-10">
          {Object.entries(groupedTopics).map(([date, dayTopics]) => (
            <div key={date} className="flex flex-col gap-4 animate-in slide-in-from-bottom-4 fade-in duration-500">
              <h2 className="text-[10px] sm:text-xs font-bold tracking-widest uppercase text-neutral-500 dark:text-neutral-500 flex items-center gap-2 sticky top-[68px] z-10 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md py-3 shadow-sm rounded-lg px-2 -mx-2">
              ...
              <div className="flex flex-col gap-3">
                {dayTopics.map((topic) => (
                  ...
                  >
                    <div className="p-5 flex items-start sm:items-center justify-between gap-4 cursor-pointer" onClick={() => toggleExpand(topic.id)}>
                      <div className="flex flex-col gap-1 min-w-0 flex-1">
                        <div className="flex items-center gap-3 flex-wrap">
                          {editingTopicId === topic.id ? (
                            <form onSubmit={(e) => saveEdit(topic.id, e)} onClick={(e) => e.stopPropagation()} className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full max-w-md">
                              <input
                                autoFocus
                                value={editTitle}
                        ...
                        <div className="text-sm font-medium text-neutral-500 flex items-center gap-2 flex-wrap">
                      ...
                      <button className={`p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-transform duration-300 shrink-0 ${expandedId === topic.id ? "rotate-180" : ""}`}>
                        ...
                        <div className="p-6 flex flex-col md:flex-row gap-8">
                          ...
                          <div className="flex-1">
                            <h4 className="text-sm font-bold text-neutral-900 dark:text-neutral-200 mb-4 flex items-center gap-2">
                              <Calendar className="w-4 h-4 text-blue-500" /> Spaced Repetition Schedule
                            </h4>
                            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3">
                              {getTimeline(topic).map((step) => (
                                <div
                                  key={step.revNumber}
                                  className={`flex flex-col items-center justify-center p-2 sm:p-3 rounded-xl border transition-all
                                  ${step.isPast ? "bg-green-50/50 dark:bg-green-900/10 border-green-200 dark:border-green-800/50 text-green-700 dark:text-green-400" : step.isNext ? (topic.isActive ? "bg-blue-500 text-white border-blue-600 shadow-md transform scale-105" : "bg-neutral-200 dark:bg-neutral-800 border-neutral-300 dark:border-neutral-700 text-neutral-500") : "bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 text-neutral-400"}`}
                                >
